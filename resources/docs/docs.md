...
The `home-routes` handler in the `choices.routes.home` namespace
...
The `home-page` function will in turn call the `choices.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/choices/core.cljs` file.



<p class="title is-5">Organizing the routes</p>

The routes are aggregated and wrapped with middleware in the `choices.handler` namespace:
...
Request middleware functions are located under the `choices.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `choices.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `choices.db.core` in some other namespace.
